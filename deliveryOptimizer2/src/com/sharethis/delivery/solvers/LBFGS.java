...
public class LBFGS {
	...
	static Logger log = Logger.getLogger(Constants.DO_LOGGER_NAME);
	...
	public static double stpmax = 1e+10;
	...
	public static void lbfgs(int n, int m, double[] x, double f, double[] g, boolean diagco, double[] diag,
			int[] iprint, double eps, double xtol, int[] iflag, String campaignName) throws ExceptionWithIflag {
		...
		if (iflag[0] == 0) {
			...
			if (gtol <= 0.0001) {
				log.error("LBFGS.lbfgs: gtol is less than or equal to 0.0001. It has been reset to 0.9.");
				gtol = 0.9;
			}
			...
			if (iprint[1 - 1] >= 0)
				lb1(iprint, iter, nfun, gnorm, n, m, x, f, g, stp, finish, campaignName);
		...
		while (true) {
			...
			String msg = Mcsrch.mcsrch(n, x, f, g, w, ispt + point * n, stp, ftol, xtol, maxfev, info, nfev, diag);
			...
			if (info[0] != 1) {
				...
				throw new ExceptionWithIflag(iflag[0],
						"Line search failed" + msg + ".\nSee documentation of routine mcsrch. Error return of line search: info = "
								+ info[0]
	...
	public static void lb1(int[] iprint, int iter, int nfun, double gnorm, int n, int m, double[] x, double f,
			double[] g, double[] stp, boolean finish, String campaignName) {
		int i;

		if (iter == 0) {
			System.out.println("***************************************************************");
			System.out.println("campaign = " + campaignName + "   n = " + n + "   number of corrections = " + m);
			...
			if (finish) {
				System.out.println("Minimization terminated without detecting errors: iflag = 0");
				System.out.println("***************************************************************");
			}
		}
		return;
