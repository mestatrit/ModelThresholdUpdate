...
RUN_DIR=${install_root}/run/iasData
...
log_file=${LOG_DIR}/log_daily_ias_process_${day}_${hour}.txt
>${log_file};
...
doneFile=ias_done.txt;
if [ "$force" == "true" ]; then
	removeFromS3 ${S3_OUTPUT_DIR}/${AGGR_DATE}/${doneFile}
fi

checkOnS3 ${S3_OUTPUT_DIR}/${AGGR_DATE}/${doneFile}
[ $? -eq 0 ] && quit "Quitting: ${S3_OUTPUT_DIR}/${AGGR_DATE}/${doneFile} exists! Job already executed!";

SUCCESS_FILE=${S3_INPUT_DIR}/${AGGR_DATE}/_SUCCESS
checkOnS3 ${SUCCESS_FILE}
[ $? -ne 0 ] && abort "Data not ready! No success file ${SUCCESS_FILE}";
...
#1.copy data from s3
file=part-*.gz
copyfroms3 ${S3_INPUT_DIR} $file ${INPUT_DATA_DIR}
if [ $? -ne 0 ];then
	abort "ERROR: S3 download for ${S3_INPUT_DIR}/${DATE}/$file to ${INPUT_DATA_DIR} failed";
fi
#unzip file
if ! gzip -d -f ${INPUT_DATA_DIR}/*.gz ; then  abort "ERROR: cannot unzip  ${INPUT_DATA_DIR}/*.gz"; fi;

#2. run server to get IAS data
log "Start download ...";
CP=${JAR_DIR}/log4j-1.2.15.jar:${JAR_DIR}/gson-2.1.jar:${JAR_DIR}/sharethis.common.jar:${JAR_DIR}/AdQuality.jar
cmd_a "java -cp ${CP} com.sharethis.adquality.IASDownloadTool.IASDownloadServer -iapp_path ${install_root} -iap $CONF_DIR/ias_downloader.properties -il4jp $CONF_DIR/log4j.properties 2>&1 >> $log_file"

[ $? -ne 0 ] && abort "Error: IASdownloaderServer returned non-zero!";

#2A. upload data to AdQuality DB
cmd_m "${BIN_DIR}/upload_to_db.py -iap $CONF_DIR/ias_downloader.properties -apb ${install_root} 2>&1 >> $log_file"

#3. upload to s3 to allow upload to memcache
outputfile=ias_data.txt
copy2S3 ${OUTPUT_DATA_DIR} $outputfile ${S3_OUTPUT_DIR}/${AGGR_DATE}
[ $? -ne 0 ] && abort "Error: copy2S3 ${OUTPUT_DATA_DIR} $outputfile ${S3_OUTPUT_DIR}/${AGGR_DATE} returned non-zero!";

> ${OUTPUT_DATA_DIR}/$doneFile;
copy2S3 ${OUTPUT_DATA_DIR} $doneFile ${S3_OUTPUT_DIR}/${AGGR_DATE}
[ $? -ne 0 ] && abort "Error: copy2S3 ${OUTPUT_DATA_DIR} $doneFile ${S3_OUTPUT_DIR}/${AGGR_DATE} returned non-zero!";
