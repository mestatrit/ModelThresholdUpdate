...
if [ -f ${missing_kpi_dates} ]
then
	rm ${missing_kpi_dates}
fi
...
java -cp ${do_runner} com.sharethis.delivery.job.Transfer -dop ${do_prop} -log4jp ${do_log} -data adx -case kpi >> ${log_output}
retvalue=$?
if [ $retvalue -ne 0 ]
then
...
if [ -s $missing_kpi_dates -a $hour -gt 9 ] 
then 
subject="[ERROR] -- Missing KPI Value Dates"

cat - ${missing_kpi_dates} << EOF | /usr/sbin/sendmail -t
To:$mailto
From:$mailfrom
Subject:$subject
...
java -cp ${do_runner} com.sharethis.delivery.job.Optimize -dop ${do_prop} -log4jp ${do_log} -pv default -date default >> ${log_output}
retvalue=$?
if [ $retvalue -ne 0 ]
then
