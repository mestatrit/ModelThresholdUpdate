...
message="$*"
echo $message
subject="[WARNING][BL Data Hourly] -- The hourly data joining job failed while processing for hour: $hh on $pDate "
...
RET_BASE_DIR="${HDFS_DIR_RET}/retarg"
RET_DATE_DIR="${RET_BASE_DIR}/${v_date}"
RET_HOUR_DIR="${RET_BASE_DIR}/${v_date_hour}"

DATA_BASE_DIR_HOURLY="${DATA_BASE_DIR}/bl_parsed_hourly"
DATA_HOUR_DIR="${DATA_BASE_DIR_HOURLY}/${v_date_hour}"

s3_data_hourly_dir=${s3_bucket_dir}/bl_parsed_hourly

data_jar=${JAR_DIR}/BrandLiftRunner.jar
data_main="com.sharethis.adoptimization.brandlift.parsing.HourlyParsingBLDataTaskMain" 
parm_date="ProcessingDate=${pDate}"
parm_hour="HourOfDay=${hh}"
parm_out_hourly="OutFilePathHourly=${DATA_BASE_DIR_HOURLY}/"
parm_ret="RetargFilePath=${RET_BASE_DIR}/"
...
data_parm="${parm_reducer} ${parm_q_name} ${parm_date} ${parm_hour} ${parm_out_hourly} ${parm_ret}"
...
then
    ...
    do
        ${HADOOP} fs -test -e ${RET_HOUR_DIR}/_SUCCESS
        if [ $? -eq 0 ]
        then
	    ...
	    then
           ...
           echo "The bl raw data under ${RET_HOUR_DIR} is not ready."
           sleep 2m
   ...
   then
        msg="Hadoop MapReduce job failed for BL Data-hourly due to that ${RET_HOUR_DIR} is not ready at `date -u +%Y%m%d%H`."
        sendalert $msg
   fi
fi
