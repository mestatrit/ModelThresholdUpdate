...
do
   ...
   do   
       ...
       echo "${BIN_DIR}/brand_lift_hourly.sh ${string_h} >> ${LOGS_FILE_PATH}/brand_lift_hourly_${string_h}.log 2>&1"
       ( ${BIN_DIR}/brand_lift_hourly.sh ${string_h} >> ${LOGS_FILE_PATH}/brand_lift_hourly_${string_h}.log 2>&1 )
       yyyy_h=${string_h:0:4}
       mm_h=${string_h:4:2}
       dd_h=${string_h:6:2}
       hh_h=${string_h:8:2}
   ...
   if [ ${delayTime} -gt 0 ] && [ ${offsetTime} -lt $HOUR_DELAY_DATA_BL_AGG ]
   then
