...
public class HourlyDataAll extends SubAssembly
{
		...
		throws IOException, ParseException, Exception
	{
		try{		
			...
			String allFilePathHourly = allFilePath + dayStr + BLConstants.hourFolders[hour] + "/" + allFileName + "/";

    		Scheme allSourceScheme = new TextDelimited(allFields, false, mDelimiter, types);
            String pipeName = "all_adg_bl";	
			...
			if(allTap.resourceExists(jobConf)){
				...
				//Filtering out the non brand lift ad groups.
				Filter<?> filterOutData = new FilterOutDataNEStrArr(BLConstants.adgIDsBL);
				allAssembly = new Each(allAssembly, new Fields("adgroup_id"), filterOutData); 
