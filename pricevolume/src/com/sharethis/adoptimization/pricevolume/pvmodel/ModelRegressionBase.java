...
public class ModelRegressionBase extends BaseOperation<ModelRegressionBase.Context> implements Aggregator<ModelRegressionBase.Context>
{
	/**
	 * 
	 */
	...
	private double maxPredWR = 0.75;
	
	public ModelRegressionBase(Fields fields, int wpThVal, double rsThVal, double maxPredWR, int numOfDays, 
			String pDateStr) {
		super(1, fields);
		this.wpThVal = wpThVal;
		this.rsThVal = rsThVal;
		this.maxPredWR = maxPredWR;
...
//		protected ArrayList<Double> wrAdjL = new ArrayList<Double>(0);
		protected int cnt = 0;
...
//		double wr = wr_tmp/(1-wr_tmp);
//		context.wrAdjL.add(context.cnt, wr);
...
//				}
			...
			if(r_square>rsThVal){
				...
				double maxPredPrice = pvMod.computingWP(beta0, beta1, maxPredWR, model_type);       
