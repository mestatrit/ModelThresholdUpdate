...
public class ModelDataWinSubAssembly extends SubAssembly
{
	private static final Logger sLogger = Logger.getLogger(ModelDataWinSubAssembly.class);
	private static final long serialVersionUID = 1L;
	private static String mDelimiter = "\t";
	
	public ModelDataWinSubAssembly(Map<String, Tap> sources, 
		...
		throws IOException, ParseException, Exception
	{
		try{		
			...
			for(int i_p=0; i_p<pipeLen; i_p++){
				String pipeName = pipeNames[i_p] + "_win_daily";
				if(numOfDays == 7)
					pipeName = pipeNames[i_p] + "_win_agg";
				winAssembly[i_p] = new Pipe(pipeName);
				// Loading the daily win data
				String pFileName = outFilePathDate + pipeName;
				sLogger.info("pFilename: " + pFileName + "   pFields: " + pFields[i_p].toString() + "    types: " + types[i_p].toString());
