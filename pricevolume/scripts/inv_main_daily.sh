...
echo "The time to start the INV main daily script: `date -u`"

while [ 1 -eq 1 ]
do
   echo "The current time is `date -u`"
   ...
   echo "The hour to start the daily ctr job: $HOUR_INV_DAILY"
   ...
   hh_inc_int=$hh_inc
   hour_daily_int=$HOUR_INV_DAILY
   if [ $hh_inc_int -ge $hour_daily_int ]
   then
       echo "The hour to start the daily inventory job: $hh_inc"
