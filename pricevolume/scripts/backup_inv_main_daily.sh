...
echo "The time to start the Inventory backup main daily script: `date -u`"
...
do
   ...
   echo "The hour to start the daily ctr job: $HOUR_INV_BKUP_DAILY"

   startingTime=`date -u +%s`
   typeset -i hh_inc_int
   typeset -i hour_daily_int
   hh_inc_int=$hh_inc
   hour_daily_int=$HOUR_INV_BKUP_DAILY
   if [ $hh_inc_int -ge $hour_daily_int ]
   then
       echo "The hour to start the daily inventory backup job: $hh_inc"
       echo "`date -u` - The daily inventory backup job of ${vDate_inc_before} started."
