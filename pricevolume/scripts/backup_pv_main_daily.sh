...
echo "The time to start the PV backup main daily script: `date -u`"
...
do
   ...
   echo "The hour to start the daily ctr job: $HOUR_PV_BKUP_DAILY"
   ...
   hh_inc_int=$hh_inc
   hour_daily_int=$HOUR_PV_BKUP_DAILY
   if [ $hh_inc_int -ge $hour_daily_int ]
   then
       echo "The hour to start the daily pv backup job: $hh_inc"
       echo "`date -u` - The daily pv backup job of ${vDate_inc_before} started."
