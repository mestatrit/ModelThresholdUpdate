...
public class ComputingPredCTRValue extends BaseOperation implements Function
{

	private static final long serialVersionUID = 1L;
	public ComputingPredCTRValue(Fields fields) {
		super(1, fields);
	}
	...
	public void operate(FlowProcess flowProcess, FunctionCall functionCall)
	{
		...
		if(arguments.getString(0) != null && arguments.getString(1) != null){
			...
			ctr = Math.max(sum_clicks,0)/Math.max(sum_imps,1);
		...
		double sum_clicks_pred = 0;
		double ctr_pred = 0;
		if(arguments.getString(3) != null){
			sum_clicks_pred = arguments.getDouble(3);
			ctr_pred = Math.max(sum_clicks_pred,0)/Math.max(sum_imps,0.0001);
			ctr_pred = Math.round(ctr_pred*1000000)/1000000.0;
		}
