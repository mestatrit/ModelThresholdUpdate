...
MOD_BASE_DIR="${HDFS_DIR}/ctr_model"
...
mod_parm="${parm_res} ${parm_q_name} ${parm_date} ${parm_in} ${parm_out} ${setHeap}"
...
if [ $cnt_sleep_mod_d -gt 1 ]
then
    msg="Hadoop MapReduce job failed for CTR model due to that ${CTR_DATE_DIR} is not ready at `date -u +%Y%m%d%H`."
    sendalert $msg
fi
fi
