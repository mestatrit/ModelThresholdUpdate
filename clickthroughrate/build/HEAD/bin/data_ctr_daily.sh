...
message="$*"
echo $message
subject="[WARNING][CTR Daily] -- The daily copying ctr old data failed while processing for day: $pDate_old"
...
s3_ctr_date_dir=${s3_backup}/${v_date_old}/ctr_daily

echo "${HADOOP} fs ${CONF} -test -e ${s3_ctr_date_dir}"
${HADOOP} fs ${CONF} -test -e ${s3_ctr_date_dir}
if [ $? -ne 0 ]
then
    s3_ctr_date_dir=${s3_backup}/${v_date_old}/ctr
fi
