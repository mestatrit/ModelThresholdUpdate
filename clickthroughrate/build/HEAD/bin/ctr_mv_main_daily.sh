...
echo "The time to start the CTR main mv daily script: `date -u`"
...
do
   ...
   echo "The hour to start the daily ctr mv job: $HOUR_CTR_DAILY"
   typeset -i hh_inc_int
   typeset -i hour_daily_int
   hh_inc_int=$hh_inc
   hour_daily_int=$HOUR_CTR_MV_DAILY
   if [ $hh_inc_int -ge $hour_daily_int ]
   then
       echo "`date -u` - The daily ctr mv job of ${vDate_inc_before} started."
