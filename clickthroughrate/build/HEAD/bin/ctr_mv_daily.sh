...
message="$*"
echo $message
subject="[WARNING][CTR Prediction Daily] -- The daily ctr prediction job failed while processing for day: $pDate"
...
CTR_BASE_DIR_HOURLY="${HDFS_DIR}/ctr_mv_hourly"
CTR_BASE_DIR_DAILY="${HDFS_DIR}/ctr_mv_daily"
...
s3_ctr_dir=${s3_model_dir}/ctr_mv_daily

s3_ctr_daily_dir=${s3_bucket_dir}/ctr_mv_daily/${v_date}

ctr_jar=${JAR_DIR}/ClickThroughRateRunner.jar
ctr_main="com.sharethis.adoptimization.clickthroughrate.ctrmodel.modelvalidation.DailyGeneratingPredCTRTaskMain"
...
do
  ...
  then
      ...
      if [ $? -ne 0 ]
      then
          echo "${BIN_DIR}/data_mv_ctr_daily.sh ${v_date_12w} >> ${LOGS_FILE_PATH}/data_mv_ctr_daily_${string12w}.log 2>&1"
          ${BIN_DIR}/data_mv_ctr_daily.sh ${v_date_12w} >> ${LOGS_FILE_PATH}/data_mv_ctr_daily_${string12w}.log 2>&1
      fi
      echo "${HADOOP} fs -test -e ${CTR_BASE_DIR_DAILY}/${v_date_4w}/_SUCCESS"
      ${HADOOP} fs -test -e ${CTR_BASE_DIR_DAILY}/${v_date_4w}/_SUCCESS
      if [ $? -ne 0 ]
      then
          echo "${BIN_DIR}/data_mv_ctr_daily.sh ${v_date_4w} >> ${LOGS_FILE_PATH}/data_mv_ctr_daily_${string4w}.log 2>&1"
          ${BIN_DIR}/data_mv_ctr_daily.sh ${v_date_4w} >> ${LOGS_FILE_PATH}/data_mv_ctr_daily_${string4w}.log 2>&1
      fi
