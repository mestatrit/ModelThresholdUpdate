...
let delay_hour=${HOUR_DELAY_DATA}
...
#string_inc=2014042407
#string_inc_after=2014042408
while true
do
   ...
   do   
       echo "The date and time for hourly copying pc data is ${string_h}"
       ...
       echo "The date and time for hourly copying pc data is ${string_h_after}"
       echo "${BIN_DIR}/data_pc_hourly.sh ${string_h_after} >> ${LOGS_FILE_PATH}/data_pc_hourly_${string_h_after}.log 2>&1"
       ( ${BIN_DIR}/data_pc_hourly.sh ${string_h_after} >> ${LOGS_FILE_PATH}/data_pc_hourly_${string_h_after}.log 2>&1 )
       echo "`date -u` - The date and time for data hourly task is ${string_h}"
       echo "${BIN_DIR}/joining_data_hourly.sh ${string_h} >> ${LOGS_FILE_PATH}/data_hourly_${string_h}.log 2>&1"
       ( ${BIN_DIR}/joining_data_hourly.sh ${string_h} >> ${LOGS_FILE_PATH}/data_hourly_${string_h}.log 2>&1 )
       yyyy_h=${string_h:0:4}
       mm_h=${string_h:4:2}
       dd_h=${string_h:6:2}
       hh_h=${string_h:8:2}
   ...
   #wait
   currentTime=`date -u`
   echo "${currentTime} - all data hourly jobs in the previous ${NUM_HOUR} hours were done."
   echo " "
   ...
   if [ ${delayTime} -gt 0 ] && [ ${offsetTime} -lt $HOUR_DELAY_DATA ]
   then
